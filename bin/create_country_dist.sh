...
awk 'match($0, /([0-9].*)/, grouping) {print grouping[0]}' "${filename::-4}"_sorted.txt >> "${filename::-4}"_data.txt
## uniq -c "${filename::-4}"_sorted.txt > "${filename::-4}"_counted.txt
## awk 'match($0, /([" "]+)([0-9]+)([" "]+)([0-9\.]+)/, grouping) {print "data.addRow([""\047"grouping[4]"\047"", "grouping[2]"]);"}' "${filename::-4}"_counted.txt >> "${filename::-4}"_data.txt
cd "$scriptloc" || exit
